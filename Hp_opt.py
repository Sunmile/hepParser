    ...
    for i in range(len(MZ_list)):
        if visited_list[i] == 0:
            ...
            if np.count_nonzero(isotopic_record[i]) >= 3 \
                    and np.sum(isotopic_record[i]) >= 5000 \
...
def get_component_mass(one_comp):
    comp_atoms = [
     ...
     #   [10, 6, 0, 5, 0]  # 甘露糖(dicp)    Man
        [12, 6, 0, 5, 0]  # 甘露糖(非dicp)    Man
    ]
...
def get_comp_pk(dir):
    if os.path.exists(dir):
        ...
        all_list = filter_component(all_list, 100, 4000)
...
def get_fit_pk(dir):
    if os.path.exists(dir):
        ...
        save_pk(point_list, './fit_data.pk')
...
def fit_all_point(point_list, degree=2):
    ...
    plt.plot(x, yvals1, 'r', label='monoisotopic peak')
    plt.scatter(x, y2, c='g')
    plt.plot(x, yvals2, 'g', label='+1 isotopic peak')
    plt.scatter(x, y3, c='b')
    plt.plot(x, yvals3, 'b', label='+2 isotopic peak')
    plt.scatter(x, y4, c='k')
    plt.plot(x, yvals4, 'k', label='+3 isotopic peak')
    plt.scatter(x, y5, c='y')
    plt.plot(x, yvals5, 'y', label='+4 isotopic peak')
    plt.legend()
...
def generate_theory_SP(compsition, prob=0.5):
    ...
    # all_com, lost_record = [compsition], [[0, 0, 0, 0]]
    # pl(all_com)
    lost_comp = []
    Z_list = []
    all_mz = []
